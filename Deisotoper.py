...
def test():
    ...
    cluster_dic_resolved = deisotoper.resolve_ambiguous(cluster_ar, mz,
                                                        intensity)
    cluster_df = deisotoper.assemble_spectrum(cluster_dic_resolved,
                                              mz, intensity, 1337)
    ...
    # =========================================================================
#    plt.bar(mz, intensity, width=0.2)
#    plt.bar([500, 501, 502, 503], exp1, width=0.2)
#    plt.bar([501, 501.5, 502], exp2, width=0.2)
#    plt.xlim(499, 504)
#    plt.show()
...
#        #debug
#        mz = mz[128:134]
#        intensity = intensity[128:134]
#
#        #desiotope
#        G = create_graph(mz, intensity, da_error)
#
#
#        #plot_graph(G)
#        G = resolve_ambigious_graph(G)
#
#        isotopes = {}
#        get_isotope_clusters(G, isotopes)
#        #resolve_shared_peaks(isotopes)
#
#        #score each cluster
#        deiso_mz = np.zeros(len(isotopes))
#        deiso_intensity = np.zeros(len(isotopes))
#        deiso_charge = np.zeros(len(isotopes))
#
#        index = 0
#        inclusion_list = []
#        exclusion_list = []
#        for k, nodes in isotopes.items():
#            charge_tmp = float(k.split(";")[1])
#            mass_tmp = mz[np.array(isotopes[k])]
#            inten_tmp =  intensity[np.array(isotopes[k])] * 1.
#            mzo, theoretical = compute_theorteical_isotopes(mass_tmp, inten_tmp, charge_tmp)
#
#            #write to datastructure
#            deiso_mz[index] = mass_tmp[0]
#            deiso_intensity[index] = inten_tmp.sum()
#            deiso_charge[index] = charge_tmp
#            index += 1
#            inclusion_list.append(nodes[0])
#            exclusion_list.extend(nodes[1:])
#            plot_isotope_comparison(mass_tmp, inten_tmp/inten_tmp.sum(), theoretical)
#            scorei = score_isotopes(mass_tmp, theoretical, charge_tmp, mass_tmp,False)
#
#        filtered_mass, filtered_intensity, filtered_charge = noise_filter(
#            mz,
#            intensity, n=topn, window_size=window_size,
#            inclusion_list=inclusion_list,
#            exclusion_list=exclusion_list,
#            inclusion_charges=deiso_charge, plot=False)
#        #overwrite current spectrum
#        line.peaks = np.array(list(zip([i*j if j != 0 else i for i,j in zip(filtered_mass, filtered_charge)], filtered_intensity)))
#        if len(filtered_charge) == 0:
#            line.peakcharge = np.zeros(line.peaks.shape[0])
#        else:
#            line.peakcharge = np.array(filtered_charge)
#        # minimum number of peaks per spectrum okay?
#        if len(deiso_mz) >= minp:
#            return(line)
#        else:
#            pass
#    except:
